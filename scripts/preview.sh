#!/usr/bin/env bash
# Regenerates docs/preview.png, the image in the README: production icon, dev icon as a launcher
...
if [[ ! -f "$generated/ic_launcher_foreground.xml" ]]; then
    echo "No generated foreground. Is a banner configured for devDebug in app/build.gradle.kts?" >&2
    exit 1
fi

# Production ships the unmodified foreground; dev ships the generated one.
python3 scripts/vd2svg.py "$work/prod.svg" "$background" "$res/ic_launcher_foreground.xml" --mask
python3 scripts/vd2svg.py "$work/dev.svg" "$background" "$generated/ic_launcher_foreground.xml" --mask
python3 scripts/vd2svg.py "$work/full.svg" "$background" "$generated/ic_launcher_foreground.xml"

for name in prod dev full; do
